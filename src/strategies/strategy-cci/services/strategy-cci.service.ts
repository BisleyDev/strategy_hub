...
@injectable()
export class StrategyCciService {
    ...
    constructor(
        @inject(StrategyCciModuleEnum.CCISignals) private _signals: SignalsService,
    ...
    private async checkSignal() {
        ...
        if (
            ...
            (this._signals.signalCloseLongPosition() || this._signals.stopLongPosition() || this._signals.stopLimitLongPosition(openPosition))
        ) {
            await this.closeLongPosition(openPosition)
        }

        if (openPosition && openPosition.side === SideOrderEnum.SELL &&
            (this._signals.signalCloseShortPosition() || this._signals.stopShortPosition() || this._signals.stopLimitShortPosition(openPosition))
        ) {
    ...
    private async openLongPosition(symbol: string) {
        if (this.instance === InstanceStrategyEnum.PROD) {
            ...
            const position: PositionInterface = {
                ...
                strategy: StrategyEnum.strategyCciPeaks,
            }
            await this._positionRepository.openPosition(position)
            this._logger.result('[cci peak] Open Long Position: ', position);
        } else {
    ...
    private async closeLongPosition(openPosition: PositionInterface) {
        ...
        openPosition.status = this._signals.signalCloseLongPosition() ? StatusPositionsEnum.CLOSED_BY_TAKE : StatusPositionsEnum.CLOSED_BY_CHANGE_TREND;
        openPosition.status = this._signals.stopLimitLongPosition(openPosition) ? StatusPositionsEnum.CLOSED_BY_STOP_LIMIT : openPosition.status
        ...
        if (this.instance === InstanceStrategyEnum.PROD) {
            ...
            this._logger.result('[cci peak] CLose position: ', openPosition)
        } else {
            const currentBar = this._indicators.currentBar;
    ...
    private async openShortPosition(symbol: string) {
        if (this.instance === InstanceStrategyEnum.PROD) {
            ...
            await this._positionRepository.openPosition(position);
            this._logger.result('[cci peak] Open Short Position: ', position);
    ...
    private async closeShortPosition(openPosition: PositionInterface) {
        openPosition.status = this._signals.signalCloseShortPosition() ? StatusPositionsEnum.CLOSED_BY_TAKE : StatusPositionsEnum.CLOSED_BY_CHANGE_TREND;
        openPosition.status = this._signals.stopLimitShortPosition(openPosition) ? StatusPositionsEnum.CLOSED_BY_STOP_LIMIT : openPosition.status
        ...
        if (this.instance === InstanceStrategyEnum.PROD) {
            ...
            await this._positionRepository.closePosition(openPosition);
            this._logger.result('[cci peak] CLose short position: ', openPosition)
        } else {
            const currentBar = this._indicators.currentBar;
            openPosition.timeClose = new Date(currentBar[BarRespEnum.closeTime]);
            openPosition.priceClose = +currentBar[BarRespEnum.closePrice];
    ...
    private async getOpenPosition(): Promise<PositionInterface | null> {
        if (this.instance === InstanceStrategyEnum.TEST) {
            ...
            return this._positionRepository.findOpenPosition(this.symbolWallet.symbol, StrategyEnum.strategyCciPeaks)
        }
    ...
    async calcResult(): Promise<string> {
        const orders = await this._positionRepository.getClosePositions(StrategyEnum.strategyCciPeaks);
