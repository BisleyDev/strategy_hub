...
import {AccountsEnum} from "../../../shared/types/accounts.enum";


@injectable()
export class StrategyRsiService {
    ...
    constructor(
        @inject(StrategyRsiModuleEnum.rsiSignals) private _signals: SignalsService,
    ...
    private async openLongPosition(symbol: string) {
        if (this.instance === InstanceStrategyEnum.PROD) {
            ...
            const position: PositionInterface = {
                ...
                strategy: StrategyEnum.strategyRSI
            }
    ...
    private async getOpenPosition(): Promise<PositionInterface | null> {
        if (this.instance === InstanceStrategyEnum.TEST) {
            ...
            return this._positionRepository.findOpenPosition(this.symbolWallet.symbol, StrategyEnum.strategyRSI)
        }
    }
